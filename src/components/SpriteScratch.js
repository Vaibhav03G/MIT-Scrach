...
import { checkCollisionAndSwap, goTo, move, resetCollisionHandled, rotate,resetSpritePositions } from "../redux/spritesSlice";
...
export default function SpriteScratch() {
    ...
    const handleReset = () => {
        dispatch(resetSpritePositions()); // Dispatch the action to reset positions
    };
    ...
    return (
        <div className="stage-area overflow-hidden relative bg-white border-2 border-gray-200" style={{ flex: 0.8 }} ref={containerRef} onDragOver={handleDragOver}
            ...
            <div className="absolute bottom-4 right-3 flex gap-5">
                <button
                    onClick={play}
